...
export class PDFService {
  ...
  async generatePromissoryNote(data: PromissoryNoteData): Promise<string> {
    return new Promise((resolve, reject) => {
      try {
        // Validate required data
        if (!data.lender?.name || !data.lender?.address || !data.lender?.jurisdiction) {
          throw new Error('Lender information is incomplete. Please configure in Settings.')
        }
        if (!data.borrower?.name || !data.borrower?.address || !data.borrower?.jurisdiction) {
      ...
      doc.text(
        `FOR VALUE RECEIVED, the undersigned (the "Borrower"), promises to pay to the order of ${data.lender.name} ` +
        ...
        `together with interest thereon at the rate of ${data.interestRate}% per annum.`,
      ...
      doc.font('Helvetica').text(
        `Interest shall accrue daily on the outstanding principal balance at an annual rate of ${data.interestRate}%, ` +
        `calculated on a 360-day year basis. Interest shall be payable monthly in arrears.`,
        { align: 'justify', lineGap: 4 }
      )
      doc.moveDown(1)
      ...
      doc.font('Helvetica').text(
        `The entire outstanding principal amount, together with all accrued and unpaid interest, shall be due and ` +
        `payable on ${this.formatDate(data.dueDate)} (the "Maturity Date").`,
        { align: 'justify', lineGap: 4 }
      )
      doc.moveDown(1)
      ...
      doc.font('Helvetica').text(
        `This Promissory Note shall be governed by and construed in accordance with the laws of ${data.lender.jurisdiction}, ` +
  ...
  async generateWireTransferOrder(data: WireTransferData): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      doc.fontSize(10).font('Helvetica')
      doc.text(
        'I hereby authorize the execution of this wire transfer as specified above.',
        70, doc.y,
        { align: 'justify' }
      )
      doc.moveDown(3)
      ...
      doc.fontSize(8).font('Helvetica-Oblique')
        .text(
          `Wire Transfer Order - Generated on ${new Date().toLocaleDateString('en-US')}`,
  ...
  async generateDebitNote(data: DebitNoteData): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      doc.text(
        `This debit note is issued for interest accrued on promissory notes as detailed above. ` +
        `Payment is due by ${this.formatDate(data.dueDate)}. Late payments may incur additional charges.`,
        50, doc.y,
      ...
      doc.fontSize(8).font('Helvetica-Oblique')
        .text(
          `Debit Note generated on ${new Date().toLocaleDateString('en-US')}`,
          50, doc.page.height - 50,
          { align: 'center' }
        )
