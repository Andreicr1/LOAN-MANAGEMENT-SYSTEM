...
exports.PromissoryNoteService = void 0;
class PromissoryNoteService {
    ...
    `);
        const row = stmt.get(disbursementId);
        if (!row)
            return null;
    ...
    createPromissoryNote(data) {
      ...
      `);
            const count = countStmt.get(`PN-${year}-%`).count;
      ...
      `);
            const result = stmt.run(data.disbursementId, pnNumber, data.principalAmount, data.interestRate, data.issueDate, data.dueDate);
            return {
                success: true,
    ...
    settlePromissoryNote(id, settlementAmount, settlementDate) {
        try {
            const stmt = this.db.prepare(`
...
exports.PromissoryNoteService = PromissoryNoteService;
