...
class DatabaseService {
    ...
    runMigrations() {
        try {
            ...
            const configCols = this.db.pragma('table_info(config)');
          ...
          `);
                ...
                try {
                    const currentConfig = this.db.prepare('SELECT docusign_integration_key FROM config WHERE id = 1').get();
            ...
            const tableInfo = this.db.pragma('table_info(clients)');
            ...
            const ensureClientColumn = (column, sql) => {
                const cols = this.db.pragma('table_info(clients)');
                if (!cols.some(col => col.name === column)) {
                    try {
                        ...
                        const alreadyExists = ((err === null || err === void 0 ? void 0 : err.message) || '').includes('duplicate column');
                        if (alreadyExists) {
